...
export const getLastRecipesList = async () => {
  try {
    const [rows] = await pool.query("select DATE_FORMAT(recl_fec, '%Y-%m-%dT%H:%i:%s') AS recl_fec, recl_rec_id from t_recipes_list where recl_fec in (select max(recl_fec) from t_recipes_list)");
